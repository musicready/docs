...
- `:beat` – rational number of quarter-notes since the start of the part (using ratio syntax, e.g., `487+1/2`).
...
- Absolute position `:at` for meter-agnostic reasoning
...
(tuplet 3:2 q                    ; Triplet: 3 in the space of 2 quarters
  (: 0 C4.e :id #uuid "..." :at 0)
  (: 0.333 D4.e :id #uuid "..." :at 0+1/3)
  (: 0.667 E4.e :id #uuid "..." :at 0+2/3))

(tuplet 5:4 q                    ; Quintuplet: 5 in the space of 4 quarters  
  (: 0 C4.s :id #uuid "..." :at 0)
  (: 0.2 D4.s :id #uuid "..." :at 0+1/5)
  (: 0.4 E4.s :id #uuid "..." :at 0+2/5)
  (: 0.6 F4.s :id #uuid "..." :at 0+3/5)
  (: 0.8 G4.s :id #uuid "..." :at 0+4/5))
```
...
(slur
  ...
  :from #uuid "018c3f1a-..."
  :to #uuid "018c3f2b-..."
  :boundary-exit :to #uuid "018c3f2b-...")
```
...
(hairpin
  ...
  :to #uuid "018c3f2d-..."
  :type crescendo
  :boundary-entry :from #uuid "018c3f1b-...")
```
...
Cross-boundary spans are write-protected at their boundary endpoints. Agents MAY modify only visual style properties that do not affect semantics. Violation is a fatal error on replace.
...
function replace(source: MRS-S, response: WorkingSetResponse) -> Result<MRS-S>:
  ...
  5. For each part/measure in scope:
